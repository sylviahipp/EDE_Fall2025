...
# could also use this character vector instead of typing in the months below 
months <- webpage %>% 
  html_nodes(".table:nth-child(7) td+ th , .table:nth-child(7) tr+ tr th") %>% 
  html_text()
months
```
...
df_withdrawals <- data.frame("Month" = month(as.Date(paste(months, 1, 2020), format = "%b %d %Y")),
                             "Year" = rep(2020,12),
...
scrape.it <- function(the_year, the_facility){
  ...
  # OR: "Month" = as.vector(rbind(1:6, 7:12))
  df_withdrawals <- data.frame("Month" = c(1,7,2,8,3,9,4,10,5,11,6,12), #<--NOTE: DIFFERENT THAN RECORDING!
                               "Year" = rep(the_year,12),
                               "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals)) %>% 
    mutate(Registrant = !!the_registrant,
...
# subset 20 to not overload the server
