...
NTL.phys.data.PeterPaul <- NTL.phys.data %>%  
...
NTL.phys.data.1999 <- NTL.phys.data %>% 
  filter(year(sampledate) == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday.1990s <- NTL.phys.data %>% 
  filter(year(sampledate) %in% c(1990:1999), 
         lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: If you were to filter using row number you would have to know the location of each observation, which could change after arranging the dataset. Filtering allows for you to subset observations based on its values of the variables.
...
NTL.phys.data.Tuesday.July.1990s <- NTL.phys.data %>% 
...
mean_tuesday_lake_surf_temp <- NTL.phys.data.Tuesday.July.1990s %>% 
  filter(depth == 0) %>%   # filter to just the surface
  summarize(mean_surface_water_temp = mean(temperature_C)) 
# 22.46 degrees C
...
NTL.phys.data.long <- NTL.phys.data %>% 
...
NTL.phys.data.wide <- NTL.phys.data %>% 
  pivot_wider(id_cols = c("lakename", "sampledate"), 
              names_from = "depth", 
