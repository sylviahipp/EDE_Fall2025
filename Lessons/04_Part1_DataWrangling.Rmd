...
class(NTL.phys.data$sampledate)  # this is now a Date object
```
...
summary(NTL.phys.data$lakename) %>% sort(decreasing = TRUE)  # Peter Lake and Paul Lake are the two most common
...
NTL.phys.data.Tuesday.1999 <- NTL.phys.data %>% 
...
> Answer: we don't filter using row numbers because we don't know which rows correspond to which year / lake name, etc.
...
NTL.phys.data.temp.desc <- NTL.phys.data %>% arrange(desc(temperature_C))
NTL.phys.data.temp.desc %>% distinct(lakename) %>% head()
NTL.phys.data.temp.desc %>% distinct(sampledate) %>% head()
