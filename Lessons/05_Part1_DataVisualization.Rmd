...
PeterPaul.chem.nutrients.gathered <-
  read.csv(here("Data/Processed/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv"),
           stringsAsFactors = TRUE)
EPAair_import <- read.csv(here("Data/Processed/EPAair_O3_PM25_NC1819_Processed.csv"), 
                          stringsAsFactors = TRUE)

EPAair_import$date <- ymd(EPAair_import$date)
...
# rename some cols to match the key
EPAair <- EPAair_import %>% 
  rename("Ozone" = aqi_value_Ozone, 
         "Date" = date, 
         "Year" = year, 
         "Month" = month,
         "Site.Name" = site_name, 
         "PM2.5" = aqi_value_PM2.5)
...
O3plot2 <- ggplot(EPAair) +
  ...
  # include constant aesthetics outside of aes()
  geom_point(aes(x = Date, y = Ozone), color = "blue")  
print(O3plot2)
...
PMplot <- 
  # make Year a factor so that it's categorical to assign shapes
  ggplot(EPAair, aes(x = Month, y = PM2.5, 
                     shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year)   # columns are years, rows are each site_name
