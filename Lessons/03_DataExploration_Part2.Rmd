...
>Answer: Data exploration is important to have a clear picture of your data (both the information it contains and any potential limits or biases) so that you can use that context when going into analysis. You want to be aware of limits in the data so you don't make any overstatements when relying on it. 
...
# 3. Import datasets
USGS.flow.data <- read.csv(here('Data/Processed/USGS_Site02085000_Flow_Processed.csv'), 
...
#View(USGS.flow.data)  # this is commented out b/c otherwise you couldn't knit
...
class(USGS.flow.data$datetime)  # imported it as a Date object
...
ggplot(data = USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()  # useful for categorical

# can call levels(USGS.flow.data$discharge.mean.approval) to get possible levels of the factor
# represents whether these values have been approved or not according to Metadata
# Conclusion: Most of the observations have been approved; some missing data
```
...
# 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean))  # can put aes() in ggplot() or geom
...
ggplot(USGS.flow.data) +
  ...
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  linetype = 2) +     # linetype argument can also be lty
  scale_x_continuous(limits = c(0, 10))

# this plot is showing counts -- reason why we see mean higher than max
# indicates that more observations have non-zero mean than max

#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean, color = gage.height.mean.approval), bins = 50) +
  scale_x_continuous(limits = c(0, 10)) +
  theme(legend.position = "bottom")  # can set position "top" "left" "right" etc.
...
# separate plots for different approval categories

#
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, 
                   y = discharge.mean, 
...
> Answer: bar plot shows distribution of categorical variable; histogram shows distribution of continuous variable. These are only useful if looking at one variable at a time. Frequency polygon shows the same as histogram, but can be useful if wanting to view distribution of several continuous variables on the same scale at one time. Box-and-whisker / violin plots both show distribution and outliers of a continuous variable, and can also be useful to see how this distribution varies in relation to another variable that you can input as your x aesthetic. Scatter plots are useful for time series or for looking at the correlation between two varibles in the data. 

## Ending discussion
What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: I learned that there's a lot of missing data. 
