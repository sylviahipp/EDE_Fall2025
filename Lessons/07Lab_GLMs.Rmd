...
EPAair_to_plot <- EPAair %>% 
  mutate(year_factor = factor(Year))

ggplot(EPAair_to_plot, aes(x = Ozone, fill = year_factor)) + 
  geom_density(alpha = 0.5) + 
  scale_fill_brewer(palette = "Set1")

ggplot(EPAair_to_plot, aes(x = year_factor, y = Ozone)) + 
  geom_boxplot() + 
  # want to show values that are very different (ex. maximums in this case)
...
pm25_by_ozone <- lm(PM2.5 ~ Ozone, data = EPAair)
summary_pm25_ozone <- summary(pm25_by_ozone)
...
ggplot(EPAair, aes(x = Ozone, y = PM2.5)) + 
  ...
  geom_text(x = 100, y = 75, label = "PM2.5 = 15.64 + 0.38*Ozone")
...
EPAair_regression_all <- lm(data = EPAair, 
                            Ozone ~ mean_Lat + mean_Lng + Month + Year + PM2.5)
step(EPAair_regression_all)
