...
theURL <- glue::glue(
  'https://waterdata.usgs.gov/nwis/uv?search_site_no={site}&period={period_days}&format=rdb')
...
scrape <- function(element, website){
  data <- website %>% 
    html_nodes(element) %>% 
    html_text
...
df_state_electricity <- 
  map(c("state" = the_states, 
        "avg_retail_price" = the_price, 
        "net_summer_cap" = the_capacity, 
        "net_generation" = net_generation, 
        "total_retail_sales" = total_retail), 
      scrape, 
      website = the_website) %>% 
  bind_cols() %>% 
  mutate(across(c(-state), ~str_remove_all(., "\\,") %>% as.numeric())) %>% 
  filter(!str_detect(tolower(state), "total"))
