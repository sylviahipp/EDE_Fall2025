...
my_plot <- nutrient_data %>% 
  ...
  scale_shape_manual(values = c(17, 15)) +
  labs(
    title = glue::glue("Concentration of {the_parameter} over time, by depth and lake"),
    x = "Date", 
    y = expression(Concentration ~ (mu*g / L)), 
    shape = "Lake", 
    color = "Depth (m)") +
...
plot_it(the_parameter = "po4", start_year = 1995, end_year = 2022)
...
selectInput(
  ...
  label = "Select the nutrient to plot:", 
  choices = c("tn_ug", "tp_ug", "no23", "po4"), 
  selected = "no23"     # default choice
)

# Create a sliderInput to set the date range
sliderInput(
  inputId = "year_slider", 
  label = "Select start and end year to plot:",
...
renderPlot(
  plot_it(the_parameter = input$the_nutrient, 
          start_year = input$year_slider[1], 
          end_year = input$year_slider[2])
)

renderText(
  glue::glue("You selected {input$the_nutrient}")
)
```
