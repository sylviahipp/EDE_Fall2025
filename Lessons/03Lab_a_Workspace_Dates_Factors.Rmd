...
ntl_data_raw <- read.csv(here("Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"), 
                         stringsAsFactors = TRUE)

# 2.
#library(tidyverse);library(here)

epa_files <- list.files(here("Data/Raw/"), 
                        pattern = "^EPAair_O3.*csv", 
                        full.names = TRUE)   # can use list.files() or dir()

epa_data_raw <- epa_files %>% 
  map(read.csv) %>% 
  bind_rows()
...
# be aware of century issue anytime you see 2 digits for the year
str_past <- "55-feb-3"
