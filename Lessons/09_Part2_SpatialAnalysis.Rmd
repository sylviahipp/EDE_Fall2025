...
huc8_upper_neuse <- huc8_sf %>% 
  filter(SUBBASIN == "Upper Neuse")

#Create a map
ggplot() + 
  geom_sf(data = huc8_sf) + 
  geom_sf(data = huc8_upper_neuse, fill = "yellow", color = "blue")
...
wind_df <- read_csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv') %>% 
  mutate(huc8 = str_extract(HUC_12, "[0-9]{8}")) %>% 
  group_by(huc8) %>% 
  summarize(total_wind_energy = sum(AvgWindEnergy)) %>% 
  ungroup()
  
#Join to HUC_8 features
wind_huc8_sf <- huc8_sf %>% 
  left_join(wind_df, 
            by = c("HUC_8" = "huc8"))

#View the outputs
ggplot(wind_huc8_sf) + 
  geom_sf(aes(fill = total_wind_energy))
...
triangle_huc8 <- huc8_sf_utm %>% 
  st_filter(triCo, .predicate = st_intersects)

mapview(triCo) + mapview(triangle_huc8, alpha.regions = 0, color = "red")

#Intersect the HUC_8s
tri_huc8_intersection <- st_intersection(triCo, huc8_sf_utm)
mapview(tri_huc8_intersection)
...
# create a buffer of 30 km and then use that buffer polygon to find intersect
