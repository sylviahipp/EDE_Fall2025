...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025,
    # expression allows you to enter equations
    # show mu as greek letter and ^{2} as exponent
    #label=expression(paste("Mean is ", mu,",", R^{2},"=0.6"))
    label="paste('Mean is ', mu,', ', R^{2},'=0.6')", 
    parse = TRUE   # parse converts the character string into an expression
    )
...
class(PeterPaul.chem.nutrients$month)  # integers

#List unique values
unique(PeterPaul.chem.nutrients$month)  # [1]  5  6  7  8  9 10 11  2

#Plot how many Lake observations occur each month
ggplot(PeterPaul.chem.nutrients) + 
  geom_bar(aes(x = month))  # may want to add missing levels

#Convert month to a factor -- with 12 levels, labelled with month names
PeterPaul.chem.nutrients$month <- PeterPaul.chem.nutrients$month %>% 
  factor(levels = c(1:12), labels = month.abb)
...
surface_temp_1999_plot + 
  ...
  labs(color='Lake') +  # change the label of the legend 
  theme_wsj()
```
...
# use this theme in the rest of the plots for the lab
my_theme <- theme_base() + 
  theme(
    line = element_line(
      color='red',
...
plot_1 <- ggplot(PeterPaul.chem.nutrients %>% filter(depth == 0)) + 
...
plot_2 <- ggplot(PeterPaul.chem.nutrients) + 
...
plot_3 <- ggplot(EPAair, 
                 aes(x = PM2.5, 
                     y = Ozone)) + 
  geom_point(aes(color = mean_Lat), 
             alpha = 0.5) + 
