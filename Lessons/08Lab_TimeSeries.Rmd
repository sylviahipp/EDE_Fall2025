...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>%  # Separate statistics col
...
BR_complete <- full_join(Rainfall_BR_processed,Temp_BR_processed)
...
# create seq of all days
months <- seq(first(BR_complete$Date), last(BR_complete$Date), by = "month") %>% 
  tibble() 
names(months) <- "Date"

BR_complete <- left_join(months, BR_complete, by = "Date")
summary(BR_complete) # no NAs
...
# unable to see a clear trend 
# see seasonality each year 
ggplot(BR_complete, aes(x = Date, y = Rainfall)) + 
  geom_line() + 
  geom_smooth(method = "lm", se = FALSE)


# see a clear positive trend
...
ts_rainfall <- ts(BR_complete$Rainfall, 
                  start = c(first_year, first_month),  
                  frequency = 12)

ts_temperature <- ts(BR_complete$Temperature_C, 
                     start = c(first_year, first_month), 
                     frequency = 12)
...
ts_rainfall_decomposed <- stl(ts_rainfall, s.window = "periodic")
plot(ts_rainfall_decomposed)

ts_temperature_decomposed <- stl(ts_temperature, s.window = "periodic")
plot(ts_temperature_decomposed)
...
ts_rainfall_components <- ts_rainfall_decomposed$time.series[ , 1:3] %>% 
  as.data.frame() %>% 
  mutate(date = BR_complete$Date, 
         observed = BR_complete$Rainfall) %>% 
  relocate(c("date", "observed"))

ts_temperature_components <- ts_temperature_decomposed$time.series[ , 1:3] %>% 
  as.data.frame() %>% 
  mutate(date = BR_complete$Date, 
...
ts_rainfall_components %>% 
  select(date, observed, trend) %>% 
  pivot_longer(cols = c("observed", "trend"), 
               names_to = "component", 
               values_to = "Rainfall") %>% 
  ggplot(aes(x = date, y = Rainfall, color = component)) + 
  geom_line() + 
  scale_color_manual(values = c("darkgrey", "red"))

ts_temperature_components %>% 
  select(date, observed, trend) %>% 
  pivot_longer(cols = c("observed", "trend"), 
               names_to = "component", 
               values_to = "Temperature_C") %>% 
  ggplot(aes(x = date, y = Temperature_C, color = component)) + 
  geom_line() + 
  scale_color_manual(values = c("darkgrey", "red"))
...
rainfall_seasonal_trend <- trend::smk.test(ts_rainfall)
rainfall_seasonal_trend
summary(rainfall_seasonal_trend)

temp_seasonal_trend <- trend::smk.test(ts_temperature)
temp_seasonal_trend
summary(temp_seasonal_trend)

# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9.

ts_rainfall_trend <- ts_rainfall_decomposed$time.series[,2]  # just extract the trend
rainfall_trend_test <- trend::mk.test(ts_rainfall_trend)
rainfall_trend_test
summary(rainfall_trend_test)

ts_temperature_trend <- ts_temperature_decomposed$time.series[,2]  # just extract the trend
temperature_trend_test <- trend::mk.test(ts_temperature_trend)
temperature_trend_test
